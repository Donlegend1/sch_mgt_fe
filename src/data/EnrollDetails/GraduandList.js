...
export const GraduandListData = () => {
  ...
  const getAllData = async () => {
    ...
    const resendMail = async (row) => {
         ...
         console.log('====================================');
       await endpoint
         .get(`/graduand/resend-payment/${row.id}`)
         .then((res) => {
             console.log("mail sent", res.data.data);
...
;
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "130px",
      cell: (row) => (
        <h6 className="fs-12 fw-bold">{row.payment_url}</h6>
      ),
    },
    {
    ...
    // },
    {
      ...
      cell: (row) => (
        <h6 className="fs-12 fw-bold">
          {row.submit_status == 0 || row.submit_status == null
            ? `Pending`
            : `Submitted`}
        </h6>
      ),
    },
    {
      ...
      cell: (row) => (
        <div className="fs-12 fw-bold mx-1 ">
          <td>
            <button className="btn btn-sm btn-primary m-1" variant="secondary" title="Action" size="sm"
              onClick={() => {
  ...
  const handleSearch = async () => {
    ...
    setResultByBatch("");
    await endpoint
      .post(`/graduand/search-graduand-by-year-of-graduation`, details)
      .then(({ data }) => {
        setLoading(false);
        setData(data.data);
      })
  ...
  const handleSearchByBatch = async () => {
    ...
    await endpoint
      .post(`/import/users/search/batch-number`, batchSearch)
      .then(({ data }) => {
        setLoading(false);
        setData(data.data);
        if (
